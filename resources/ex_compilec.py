...
class SymbolTable(object):
    ...
    def display(self):
        ...
        # print table header
        print "{0:3s} | {1:^{2}s} | {3:^{4}s} | {5:^{6}s} | {7:^{8}} | {9:s}".format(" No", sym_name, sym_len,
                                                                                     kind_name, kind_len, type_name,
                                                                                     type_len, attr_name, attr_len,
                                                                                     "Parameters")
        ...
        for i, sym in enumerate(self.table):
            ...
            for p in sym.param_types:
                if parameters == "":
                    ...
                    parameters += ", {0}".format(SharedData.TYPES[p])
            print "{0:3d} | {1:^{2}s} | {3:^{4}s} | {5:^{6}s} | {7:^{8}} | ({9})".format(i, sym.name, sym_len,
                                                                                         SharedData.KINDS[sym.kind],
                                                                                         kind_len,
                                                                                         SharedData.TYPES[sym.type],
                                                                                         type_len, sym.attribute_str(),
                                                                                         attr_len, parameters)
    ...
    def insert_global_var(self, vname, vtype):
        ...
        return self.insert_id(vname, SharedData.KINDS.GLOBAL_VAR,
                              [SharedData.KINDS.GLOBAL_VAR, SharedData.KINDS.FUNCTION], vtype)

    def insert_local_var(self, vname, vtype, position):
        "Inserts a new local variable"
        index = self.insert_id(vname, SharedData.KINDS.LOCAL_VAR,
                               [SharedData.KINDS.LOCAL_VAR, SharedData.KINDS.PARAMETER], vtype)
    ...
    def insert_function(self, fname, ftype):
        ...
        index = self.insert_id(fname, SharedData.KINDS.FUNCTION,
                               [SharedData.KINDS.GLOBAL_VAR, SharedData.KINDS.FUNCTION], ftype)
...
class CodeGenerator(object):
    ...
    def arithmetic(self, operation, operand1, operand2, operand3=None):
        ...
        self.newline_text(
            "{0}\t{1},{2},{3}".format(mnemonic, self.symbol(operand1), self.symbol(operand2), self.symbol(output)),
            True)
        return output
    ...
    def compare(self, operand1, operand2):
        ...
        self.free_if_register(operand1)
        self.free_if_register(operand2)
        self.newline_text("CMP{0}\t{1},{2}".format(self.OPSIGNS[typ], self.symbol(operand1), self.symbol(operand2)),
                          True)
...
class MicroC(object):
    ...
    def __init__(self):
        ...
        self.rFunctionCall = ((self.tId("name") + FollowedBy("(")).setParseAction(self.function_call_prepare_action) +
                              Suppress("(") + Optional(self.rArguments)("args") + Suppress(")")).setParseAction(
        ...
        self.rAndExp << (self.rRelExp("exp") + ZeroOrMore(Literal("&&").setParseAction(self.andexp_action) +
                                                          self.rRelExp("exp")).setParseAction(
            lambda x: self.relexp_code))
        self.rLogExp << (self.rAndExp("exp") + ZeroOrMore(Literal("||").setParseAction(self.logexp_action) +
                                                          self.rAndExp("exp")).setParseAction(
            lambda x: self.andexp_code))
        ...
        self.rLocalVariable = (self.tType("type") + self.tId("name") + FollowedBy(";")).setParseAction(
            self.local_variable_action)
        self.rLocalVariableList = ZeroOrMore(self.rLocalVariable + Suppress(";"))
        self.rFunctionBody = Suppress("{") + Optional(self.rLocalVariableList).setParseAction(
        ...
        self.rProgram = (Empty().setParseAction(self.data_begin_action) + self.rGlobalVariableList +
                         Empty().setParseAction(self.code_begin_action) + self.rFunctionList).setParseAction(
            self.program_end_action)
    ...
    def lookup_id_action(self, text, loc, var):
        ...
        if DEBUG > 0:
            ...
            if DEBUG == 2: self.symtab.display()
            if DEBUG > 2: return
        var_index = self.symtab.lookup_symbol(var.name, [SharedData.KINDS.GLOBAL_VAR, SharedData.KINDS.PARAMETER,
    ...
    def assignment_action(self, text, loc, assign):
        ...
        if DEBUG > 0:
            ...
            if DEBUG == 2: self.symtab.display()
            if DEBUG > 2: return
        var_index = self.symtab.lookup_symbol(assign.var, [SharedData.KINDS.GLOBAL_VAR, SharedData.KINDS.PARAMETER,
                                                           SharedData.KINDS.LOCAL_VAR])
        if var_index == None:
    ...
    def argument_action(self, text, loc, arg):
        ...
        if not self.symtab.same_type_as_argument(arg.exp, self.function_call_index, arg_ordinal):
            raise SemanticException("Incompatible type for argument %d in '%s'" % (
